...
weekday_full = ['Monday', 'Tuesday', 'Wednesday',
                'Thursday', 'Friday', 'Saturday', 'Sunday']
weekday_abbr = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
month_abbr = [None, 'Jan', 'Feb', 'Mar', 'Apr', 'May',
...
class TestDateParser(unittest.TestCase):
    ...
    def test_asctime(self):
        ...
        rs = time.strftime("%%s %%s %d %H:%M:%S %Y", t) % (
            weekday_abbr[t.tm_wday], month_abbr[t.tm_mon])
        self.assertEqual(int(ts), int(parse_date(rs)))
...
class TestSendFile(unittest.TestCase):
    ...
    def test_mime(self):
        ...
        self.assertTrue(f.headers['Content-Type'].split(';')[0]
        ...
        f = static_file(basename, root=root,
                        mimetype='text/foo', charset='latin1')
        self.assertEqual('text/foo; charset=latin1', f.headers['Content-Type'])

    def test_ims(self):
        """ SendFile: If-Modified-Since"""
        request.environ['HTTP_IF_MODIFIED_SINCE'] = bottle.http_date(
            time.time())
        ...
        self.assertAlmostEqual(
            int(time.time()), parse_date(res.headers['Date']))
        request.environ['HTTP_IF_MODIFIED_SINCE'] = bottle.http_date(100)
        self.assertEqual(open(__file__, 'rb').read(),
    ...
    def test_download(self):
        ...
        self.assertEqual('attachment; filename="%s"' %
                         basename, f.headers['Content-Disposition'])
        request.environ['HTTP_IF_MODIFIED_SINCE'] = bottle.http_date(100)
    ...
    def test_range_parser(self):
        def r(rs): return list(parse_range_header(rs, 100))
